...
<script>
import { getShareRegister } from '@/api/user'
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { title: '注册时间', key: 'createtime', render: (h, { row: { createtime } }) => h('span', formatDate(createtime * 1000)) },
  ...
  methods: {
    ...
    getList (page) {
      ...
      this.loading = true
      getShareRegister(pageNum, pageSize).then(data => {
        this.data = data.list
